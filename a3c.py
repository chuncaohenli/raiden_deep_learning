...
def actor_learner_thread(num, env, session, graph_ops, summary_ops, saver):
    ...
    global TMAX, T, SAVE_NUM, SAVE_DELTA
    ...
    while T < TMAX:
        ...
        while not (terminal or ((t - t_start)  == t_max)):
            ...
            if probs_summary_t % 100 == 0:
                print ("P, ", np.max(probs), "V ", session.run(v_network, feed_dict={s: [s_t]})[0][0], "T ", T)

            s_batch.append(s_t)
            a_batch.append(a_t)

            s_t1, r_t, terminal, score, hp, live = env.step(a_t)
            ep_reward += r_t
...
def train(session, graph_ops, saver):
    ...
    envs = [Raiden_Env() for i in range(NUM_CONCURRENT)]
