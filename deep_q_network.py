...
def trainNetwork(s, readout, h_fc1, sess):
    ...
    # printing
    a_file = open(os.path.join("logs_" + GAME, "readout.txt"), 'w')
    h_file = open(os.path.join("logs_" + GAME, "hidden.txt"), 'w')

    # get the first state by doing nothing and preprocess the image to 80x80x4
    game_state.reset()
    x_t, r_0, terminal = game_state.step(8)
